...
def test_other(a: float, b: float) -> None:
    ...
    if abs(a) > 1e-5 and abs(b) > 1e-5:
        assert_close(inv(mul(a, b)), mul(inv(a), inv(b)))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sum_of_sums = minitorch.operators.sum(ls1) + minitorch.operators.sum(ls2)
    
    sum_of_elements = minitorch.operators.sum(minitorch.operators.addLists(ls1, ls2))
